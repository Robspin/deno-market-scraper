...
const determineIchimokuSentiment = (state: IchimokuState): Sentiment => {
    const { priceAboveCloud, bullishCross, bullishCloud, priceBelowCloud } = state

    if (priceAboveCloud && bullishCross && bullishCloud) return sentimentObject['bullish']
    if (priceBelowCloud && !bullishCross && !bullishCloud) return sentimentObject['bearish']
